      ...
      <div className="grid grid-cols-1 gap-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-3 3xl:grid-cols-4">
        {isLoading ? (
          ...
          Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-72 md:h-80 rounded-[32px] skeleton-shimmer"></div>
          ))
        ) : (
           displayedSetups.map((setup) => {
             ...
             <Link key={setup.id} href={`/combinations/${setup.id}`}>
               <div className="bg-[#f9f9fa] rounded-[32px] p-6 cursor-pointer hover:bg-white hover:border hover:border-[#e1e3e6] transition-all group">
                    ...
                    {setup.setup_blocks
                      ...
                      return (
                        <div key={block.id || index} className="flex flex-col items-center w-[100px] h-[100px] md:w-[140px] md:h-[140px]">
                                                      <div className="w-[100px] h-[84px] md:w-[140px] md:h-[124px] p-2 md:p-3 flex items-center justify-center">
